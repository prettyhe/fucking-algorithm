...
其实回溯算法和我们常说的 DFS 算法非常类似，本质上就是一种暴力穷举算法。回溯算法和 DFS 算法的细微差别是：回溯算法是在遍历「树枝」，DFS 算法是在遍历「节点」，本文就是简单提一下，你有个印象就行了。等你看了 [手把手刷二叉树（纲领篇）](./../数据结构系列/二叉树总结.md) 和 [图论算法基础](./../数据结构系列/图.md) 之后就能深刻理解这句话的含义了。
...
> info：**我们这次讨论的全排列问题不包含重复的数字，包含重复数字的扩展场景我在后文 [回溯算法秒杀排列组合子集的九种题型](./../高频面试系列/子集排列组合.md) 中讲解**。
...
再进一步，如何遍历一棵树？这个应该不难吧。回忆一下之前 [学习数据结构的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 写过，各种搜索问题其实都是树的遍历问题，而多叉树的遍历框架就是这样：
...
>
> 是的，DFS 算法的前序和后序位置应该在 for 循环外面，不过回溯算法和 DFS 算法略有不同，后文 [图论算法基础](./../数据结构系列/图.md) 会详细对比，这里可以暂且忽略这个问题。
...
当然，因为有 `isValid` 函数剪枝，并不会真的在每个位置都尝试放皇后，所以实际的执行效率会高一些。但正如前文 [算法时空复杂度分析实用指南](https://labuladong.gitee.io/article/fname.html?fname=时间复杂度) 所说，这个时间复杂度作为上界是没问题的。
...
这样修改后，只要找到一个答案，后续的递归穷举都会被阻断。也许你可以在 N 皇后问题的代码框架上，稍加修改，写一个解数独的算法？可以参考我的这篇文章 [回溯算法秒杀数独问题](https://labuladong.gitee.io/article/fname.html?fname=sudoku)。
...
动态规划和回溯算法底层都把问题抽象成了树的结构，但这两种算法在思路上是完全不同的。在 [东哥带你刷二叉树（纲领篇）](./../数据结构系列/二叉树总结.md) 你将看到动态规划和回溯算法更深层次的区别和联系。
...
<summary><strong>引用本文的文章</strong></summary>
 ...
 - [FloodFill算法详解及应用](https://labuladong.gitee.io/article/fname.html?fname=FloodFill算法详解及应用)
