...
根据 [学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md)，完全可以根据二叉树的遍历框架改写出二维矩阵的 DFS 代码框架：
...
这里额外说一个处理二维数组的常用小技巧，你有时会看到使用「方向数组」来处理上下左右的遍历，和前文 [union-find 算法详解](./../算法思维系列/UnionFind算法详解.md) 的代码很类似：
...
> tip：这类 DFS 算法还有个别名叫做 [FloodFill 算法](https://labuladong.gitee.io/article/fname.html?fname=FloodFill算法详解及应用)，现在有没有觉得 FloodFill 这个名字还挺贴切的~
...
> tip：处理这类岛屿题目除了 DFS/BFS 算法之外，Union Find 并查集算法也是一种可选的方法，前文 [Union Find 算法运用](./../算法思维系列/UnionFind算法详解.md) 就用 Union Find 算法解决了一道类似的问题。
...
**这道题的关键在于，如何快速判断子岛屿**？肯定可以借助 [Union Find 并查集算法](./../算法思维系列/UnionFind算法详解.md) 来判断，不过本文重点在 DFS 算法，就不展开并查集算法了。
...
如果想把岛屿转化成字符串，说白了就是序列化，序列化说白了就是遍历嘛，前文 [二叉树的序列化和反序列化](https://labuladong.gitee.io/article/fname.html?fname=二叉树的序列化) 讲了二叉树和字符串互转，这里也是类似的。
...
> note：仔细看这个代码，在递归前做选择，在递归后撤销选择，它像不像 [回溯算法框架](./../算法思维系列/回溯算法详解修订版.md)？实际上它就是回溯算法，因为它关注的是「树枝」（岛屿的遍历顺序），而不是「节点」（岛屿的每个格子）。
...
这样，这道题就解决了，至于为什么初始调用 `dfs` 函数时的 `dir` 参数可以随意写，因为这个 `dfs` 函数实际上是回溯算法，它关注的是「树枝」而不是「节点」，前文 [图算法基础](./../数据结构系列/图.md) 有写具体的区别，这里就不赘述了。
