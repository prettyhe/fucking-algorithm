...
前文 [数组双指针技巧汇总](./../算法思维系列/双指针技巧.md) 和 [子序列问题解题思路](./../动态规划系列/子序列问题模板.md) 讲解了回文串和回文序列相关的问题，先来简单回顾下。
...
而**判断**一个字符串是不是回文串就简单很多，不需要考虑奇偶情况，只需要[双指针技巧](./../算法思维系列/双指针技巧.md)，从两端向中间逼近即可：
...
那么最简单的办法就是，把原始链表反转存入一条新的链表，然后比较这两条链表是否相同。关于如何反转链表，可以参见前文 [递归翻转链表的一部分](./../数据结构系列/递归反转链表的一部分.md)。
...
在 [学习数据结构的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 中说过，链表兼具递归结构，树结构不过是链表的衍生。那么，**链表其实也可以有前序遍历和后序遍历**：
...
**1、先通过 [双指针技巧](https://labuladong.gitee.io/article/fname.html?fname=链表技巧) 中的快慢指针来找到链表的中点**：
