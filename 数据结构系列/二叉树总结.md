...
我的公众号历史文章的整个脉络都是按照 [学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 提出的框架来构建的，其中着重强调了二叉树题目的重要性，所以把本文放在第一章的必读系列中。
...
本文中会用题目来举例，但都是最最简单的题目，所以不用担心自己看不懂，我可以帮你从最简单的问题中提炼出所有二叉树题目的共性，并将二叉树中蕴含的思维进行升华，反手用到 [动态规划](./../动态规划系列/动态规划详解进阶.md)，[回溯算法](./../算法思维系列/回溯算法详解修订版.md)，[分治算法](https://labuladong.gitee.io/article/fname.html?fname=分治算法)，[图论算法](./../数据结构系列/图.md) 中去，这也是我一直强调框架思维的原因。希望你在学习了上述高级算法后，也能回头再来看看本文，会对它们有更深刻的认识。

首先，我还是要不厌其烦地强调一下二叉树这种数据结构及相关算法的重要性。

### 二叉树的重要性

举个例子，比如两个经典排序算法 [快速排序](https://labuladong.gitee.io/article/fname.html?fname=快速排序) 和 [归并排序](https://labuladong.gitee.io/article/fname.html?fname=归并排序)，对于它俩，你有什么理解？
...
首先，回顾一下 [学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 中说到的二叉树遍历框架：
...
你也可以看到，[图论算法基础](./../数据结构系列/图.md) 把二叉树的遍历框架扩展到了图，并以遍历为基础实现了图论的各种经典算法，不过这是后话，本文就不多说了。

### 两种解题思路

前文 [我的算法学习心得](https://labuladong.gitee.io/article/fname.html?fname=算法心得) 说过：

**二叉树题目的递归解法可以分两类思路，第一类是遍历一遍二叉树得出答案，第二类是通过分解问题计算出答案，这两类思路分别对应着 [回溯算法核心框架](./../算法思维系列/回溯算法详解修订版.md) 和 [动态规划核心框架](./../动态规划系列/动态规划详解进阶.md)**。

> tip：这里说一下我的函数命名习惯：二叉树中用遍历思路解题时函数签名一般是 `void traverse(...)`，没有返回值，靠更新外部变量来计算结果，而用分解问题思路解题时函数名根据该函数具体功能而定，而且一般会有返回值，返回值是子问题的计算结果。
>
> 与此对应的，你会发现我在 [回溯算法核心框架](./../算法思维系列/回溯算法详解修订版.md) 中给出的函数签名一般也是没有返回值的 `void backtrack(...)`，而在 [动态规划核心框架](./../动态规划系列/动态规划详解进阶.md) 中给出的函数签名是带有返回值的 `dp` 函数。这也说明它俩和二叉树之间千丝万缕的联系。
>
...
![](https://labuladong.gitee.io/pictures/二叉树收官/plugin1.jpg)
...
其实我在 [一文秒杀所有岛屿题目](./../高频面试系列/岛屿题目.md) 中就是用的 DFS 算法，但我确实没有单独用一篇文章讲 DFS 算法，**因为 DFS 算法和回溯算法非常类似，只是在细节上有所区别**。
...
你再看看具体的动态规划问题，比如 [动态规划框架套路详解](./../动态规划系列/动态规划详解进阶.md) 中举的斐波那契的例子，我们的关注点在一棵棵子树的返回值上：
...
这个多叉树的遍历框架就可以延伸出 [回溯算法框架套路详解](./../算法思维系列/回溯算法详解修订版.md) 中的回溯算法框架：
...
你再看看具体的回溯算法问题，比如 [回溯算法秒杀排列组合子集的九种题型](./../高频面试系列/子集排列组合.md) 中讲到的全排列，我们的关注点在一条条树枝上：
...
你再看看具体的 DFS 算法问题，比如 [一文秒杀所有岛屿题目](./../高频面试系列/岛屿题目.md) 中讲的前几道题，我们的关注点是 `grid` 数组的每个格子（节点），我们要对遍历过的格子进行一些处理，所以我说是用 DFS 算法解决这几道题的：
...
前文 [BFS 算法框架](./../算法思维系列/BFS框架.md) 就是从二叉树的层序遍历扩展出来的，常用于求无权图的**最短路径**问题。

当然这个框架还可以灵活修改，题目不需要记录层数（步数）时可以去掉上述框架中的 for 循环，比如前文 [Dijkstra 算法](./../数据结构系列/dijkstra算法.md) 中计算加权图的最短路径问题，详细探讨了 BFS 算法的扩展。
...
关于层序遍历（以及其扩展出的 [BFS 算法框架](./../算法思维系列/BFS框架.md)），我在最后多说几句。
