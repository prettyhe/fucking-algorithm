...
经常有读者问我「图」这种数据结构，其实我在 [学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 中说过，虽然图可以玩出更多的算法，解决更复杂的问题，但本质上图可以认为是多叉树的延伸。
...
**[学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 说过，各种数据结构被发明出来无非就是为了遍历和访问，所以「遍历」是所有数据结构的基础**。
...
如果让你处理路径相关的问题，这个 `onPath` 变量是肯定会被用到的，比如 [拓扑排序](./../数据结构系列/拓扑排序.md) 中就有运用。

另外，你应该注意到了，这个 `onPath` 数组的操作很像前文 [回溯算法核心套路](./../算法思维系列/回溯算法详解修订版.md) 中做「做选择」和「撤销选择」，区别在于位置：回溯算法的「做选择」和「撤销选择」在 for 循环里面，而对 `onPath` 数组的操作在 for 循环外面。

为什么有这个区别呢？这就是前文 [东哥带你刷二叉树（纲领篇）](./../数据结构系列/二叉树总结.md) 中讲到的回溯算法和 DFS 算法的区别所在：回溯算法关注的不是节点，而是树枝。如果没印象了，强烈建议重新阅读前文。
...
当然，图还会有很多其他的有趣算法，比如 [二分图判定](https://labuladong.github.io/article/fname.html?fname=二分图)，[环检测和拓扑排序](./../数据结构系列/拓扑排序.md)（编译器循环引用检测就是类似的算法），[最小生成树](https://labuladong.github.io/article/fname.html?fname=kruskal)，[Dijkstra 最短路径算法](./../数据结构系列/dijkstra算法.md) 等等，有兴趣的读者可以去看看，本文就到这了。
