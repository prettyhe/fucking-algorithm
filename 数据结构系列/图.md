...
当然，图还会有很多其他的有趣算法，比如 [二分图判定](https://labuladong.gitee.io/article/fname.html?fname=二分图)，[环检测和拓扑排序](./../数据结构系列/拓扑排序.md)（编译器循环引用检测就是类似的算法），[最小生成树](https://labuladong.gitee.io/article/fname.html?fname=kruskal)，[Dijkstra 最短路径算法](./../数据结构系列/dijkstra算法.md) 等等，有兴趣的读者可以去看看，本文就到这了。
