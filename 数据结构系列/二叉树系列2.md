...
本文是承接 [东哥带你刷二叉树（纲领篇）](./../数据结构系列/二叉树总结.md) 的第二篇文章，先复述一下前文总结的二叉树解题总纲：
...
第一篇文章 [东哥带你刷二叉树（思维篇）](./../数据结构系列/二叉树系列1.md) 讲了「遍历」和「分解问题」两种思维方式，本文讲二叉树的构造类问题。
...
前文 [二叉树就那几个框架](https://labuladong.gitee.io/article/fname.html?fname=nestInteger) 写过，这样的遍历顺序差异，导致了 `preorder` 和 `inorder` 数组中的元素分布有如下特点：
...
* [手把手刷二叉树（第三期）](https://labuladong.gitee.io/article/fname.html?fname=二叉树系列3)
