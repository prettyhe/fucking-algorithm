...
不过以我的经验呢，像网络流这种问题，你又不是打竞赛的，没时间的话就没必要学了；像 [最小生成树](https://labuladong.github.io/article/fname.html?fname=prim算法) 和 [最短路径问题](./../数据结构系列/dijkstra算法.md)，虽然从刷题的角度用到的不多，但它们属于经典算法，学有余力可以掌握一下；像 [二分图判定](https://labuladong.github.io/article/fname.html?fname=二分图)、拓扑排序这一类，属于比较基本且有用的算法，应该比较熟练地掌握。
...
如何转换成图呢？我们前文 [图论基础](./../数据结构系列/图.md) 写过图的两种存储形式，邻接矩阵和邻接表。
...
前文 [图论基础](./../数据结构系列/图.md) 写了 DFS 算法遍历图的框架，无非就是从多叉树遍历框架扩展出来的，加了个 `visited` 数组罢了：
...
前文 [学习数据结构和算法的框架思维](./../算法思维系列/学习数据结构和算法的高效方法.md) 说过，图的遍历和遍历多叉树差不多，所以到这里你应该都能很容易理解。

现在可以思考如何判断这幅图中是否存在环。

我们前文 [回溯算法核心套路详解](./../算法思维系列/回溯算法详解修订版.md) 说过，你可以把递归函数看成一个在递归树上游走的指针，这里也是类似的：
...
其实 BFS 算法借助 `indegree` 数组记录每个节点的「入度」，也可以实现这两个算法。不熟悉 BFS 算法的读者可阅读前文 [BFS 算法核心框架](./../算法思维系列/BFS框架.md)。
...
按道理，[图的遍历](./../数据结构系列/图.md) 都需要 `visited` 数组防止走回头路，这里的 BFS 算法其实是通过 `indegree` 数组实现的 `visited` 数组的作用，只有入度为 0 的节点才能入队，从而保证不会出现死循环。
