...
之前 [手把手带你刷二叉树（纲领篇）](./../数据结构系列/二叉树总结.md) 把递归穷举划分为「遍历」和「分解问题」两种思路，其中「遍历」的思路扩展延伸一下就是回溯算法，「分解问题」的思路可以扩展成动态规划算法。

我在 [手把手带你刷二叉树（思路篇）](./../数据结构系列/二叉树系列1.md) 对一些二叉树问题进行举例，同时给出「遍历」和「分解问题」两种思路的解法，帮大家借助二叉树理解更高级的算法设计思想。

当然，这种思维转换不止局限于二叉树相关的算法，本文就跳出二叉树类型问题，来看看实际算法题中如何把问题抽象成树形结构，从而进行「遍历」和「分解问题」的思维转换，从 [回溯算法](./../算法思维系列/回溯算法详解修订版.md) 顺滑地切换到 [动态规划算法](./../动态规划系列/动态规划详解进阶.md)。

先说句题外话，前文 [动态规划核心框架详解](./../动态规划系列/动态规划详解进阶.md) 说，**标准的动态规划问题一定是求最值的**，因为动态规划类型问题有一个性质叫做「最优子结构」，即从子问题的最优解推导出原问题的最优解。
...
这就是前文 [回溯算法秒杀排列组合问题的九种变体](./../高频面试系列/子集排列组合.md) 中讲到的最后一种变体：元素无重可复选的排列问题，前文我写了一个 `permuteRepeat` 函数，代码如下：
...
然后，按照前文 [回溯算法框架详解](./../算法思维系列/回溯算法详解修订版.md) 所说，你把 `backtrack` 函数理解成在回溯树上游走的一个指针，维护每个节点上的变量 `i`，即可遍历整棵回溯树，寻找出匹配 `s` 的组合。
...
这段代码就是严格按照回溯算法框架写出来的，应该不难理解，但这段代码无法通过所有测试用例，我们按照之前 [算法时空复杂度使用指南](https://labuladong.gitee.io/article/fname.html?fname=时间复杂度) 中讲到的方法来分析一下它的时间复杂度。
...
这个解法能够通过所有测试用例，我们根据 [算法时空复杂度使用指南](https://labuladong.gitee.io/article/fname.html?fname=时间复杂度) 来算一下它的时间复杂度：
