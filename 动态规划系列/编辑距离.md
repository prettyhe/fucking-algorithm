...
前文 [最长公共子序列](./../动态规划系列/LCS.md) 说过，**解决两个字符串的动态规划问题，一般都是用两个指针 `i, j` 分别指向两个字符串的最后，然后一步步往前移动，缩小问题的规模**。
...
**怎么能一眼看出存在重叠子问题呢**？前文 [动态规划之正则表达式](./../动态规划系列/动态规划之正则表达.md) 有提过，这里再简单提一下，需要抽象出本文算法的递归框架：
...
对于重叠子问题呢，前文 [动态规划详解](./../动态规划系列/动态规划详解进阶.md) 详细介绍过，优化方法无非是备忘录或者 DP table。
