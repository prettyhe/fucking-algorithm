...
上篇文章 [最小路径和](https://labuladong.gitee.io/article/fname.html?fname=最小路径和) 写过类似的问题，问你从左上角到右下角的最小路径和是多少。
...
这类求最值的问题，肯定要借助动态规划技巧，要合理设计 `dp` 数组/函数的定义。类比前文 [最小路径和问题](https://labuladong.gitee.io/article/fname.html?fname=最小路径和)，`dp` 函数签名肯定长这样：
...
这就是自顶向下带备忘录的动态规划解法，参考前文 [动态规划套路详解](./../动态规划系列/动态规划详解进阶.md) 很容易就可以改写成 `dp` 数组的迭代解法，这里就不写了，读者可以尝试自己写一写。
